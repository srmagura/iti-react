...
export function Pager(props: IPagerProps): JSX.Element {
    ...
    while (true) {
        ...
        if (addLeft)
            pageNumbers = [left].concat(pageNumbers)
        ...
        if (pageNumbers.length === pagesToDisplay ||
            (!addLeft && !addRight)) {
            break
        }
    ...
    return (
        <nav aria-label="Page navigation" className="pagination-container">
            <ul className="pagination">
                    <PagerLink anchorAttributes={anchorAttributesBuilder(page - 1)} key="prev" enabled={hasPrevious}>
                        <span aria-hidden="true">&laquo;</span>
                        <span className="sr-only">Previous</span>
                    </PagerLink>

                {pageNumbers.map((i: number) =>
                    <PagerLink anchorAttributes={anchorAttributesBuilder(i)} active={page === i} key={i.toString()} enabled={true}>
                        {i}
                    </PagerLink>)}

                    <PagerLink anchorAttributes={anchorAttributesBuilder(page + 1)} key="next" enabled={hasNext}>
