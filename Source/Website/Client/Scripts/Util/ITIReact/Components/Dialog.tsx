...
export const ActionDialog: React.SFC<IActionDialogProps> = props => {
    const { actionButtonText, actionButtonClass,
        action, loading, id, title, modalClass, onClose, children, focusFirst } = props

    const footer = [
        <SubmitButton type="button" onClick={action} className={`btn ${actionButtonClass}`}
                      submitting={loading}
                      key="action">
            {actionButtonText}
        </SubmitButton>,
        <button type="button" className="btn btn-secondary" data-dismiss={loading ? '' : 'modal'} key="cancel">Cancel</button>
    ]
    ...
    componentDidMount() {
        ...
        if (focusFirst) {
            el.on('shown.bs.modal', () => {
                const firstInput = el.find('input, select, textarea')
                    .filter(':not([readonly])')
                ...
                <div className={"modal-dialog " + modalClass} role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <button type="button" className="close" data-dismiss={allowDismiss ? 'modal' : undefined} aria-label="Close">
