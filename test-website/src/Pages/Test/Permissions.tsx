...
import { useParameterlessQuery, useReadiness, allReady, FormCheck, useQuery, getGuid } from '@interface-technologies/iti-react';
...
export default function Page({ ready, onReady, onError }: PageProps): React.ReactElement {
    ...
    const customerId: Identity = useMemo(() => allowedCustomerId ? { guid: "643534f0-af21-41b1-b7f2-3e76cc407dcc" } : {
        guid: '00000000-0000-0000-0000-000000000000'
    }, [allowedCustomerId])
    const vendorId: Identity = useMemo(() => allowedVendorId ? { guid: "7f1e6ef0-4559-4d6a-b3df-f78e66ceaf76" } : {
        guid: '00000000-0000-0000-0000-000000000000'
    }, [allowedVendorId])
    ...
    useQuery<Identity, { canManageCustomer: boolean }>({
        ...
        query: customerId => api.appPermissions.get({ canManageCustomer: [PermissionName.CanManageCustomer, customerId] }),
    ...
    return <div className="page-test-permissions" hidden={!ready}>
        ...
        <p className="mb-0">canViewOrders: {globalPermissions?.canViewOrders.toString()}</p>
